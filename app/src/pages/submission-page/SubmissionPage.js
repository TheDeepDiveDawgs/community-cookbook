	...
	render() {
		...
		let submissions = ingredients.map((ing, i) => (
			...
			<div className="recipe-form-line" key={`ingredient-${i}`}>
				<label> {i + 1}.
					<submission type="text" name={`ingredient-${i}`} value={ing} size={45} autoComplete="off" placeholder="Ingredient"
			...
			<form className='recipe-form' onSubmit={this.handleSubmit}>
				<button type="button" className="close-button" onClick={onClose}>X</button>
					...
					<div className='recipe-form-line'>
						<label htmlFor='recipe-title-submission'>Title</label>
		...
		<textarea key='instructions' id='recipe-instructions-submission' type='Instructions' name='instructions'
					 rows='8' cols='50' autoComplete='off' value={instructions} onChange={this.handleChange}
		/> {submissions}

				<button type="button" onClick={this.handleNewIngredient} className="buttons">+</button>

			<div className='recipe-form-line'>
				<label htmlFor='recipe-img-submission'> Image Url</label>
				<submission id='recipe-img-submission' type='text' placeholder='' name='img' value={img} size={36}
